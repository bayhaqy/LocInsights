...
import {
  ...
  Globe, Github, Download, Smartphone, Apple, WifiOff, Check, X,
} from 'lucide-react'
      ...
      <Card className="card-premium border-[var(--brand-red)]/30">
        <CardHeader className="pb-3">
          ...
          <p className="text-[12px] text-[var(--brand-ink)]/60 mt-1">
            {t('about.download_app.subtitle')}
          </p>
        </CardHeader>
        <CardContent className="pt-0 space-y-4">
          {/* Three platform cards: PWA, Android, iOS */}
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
            {/* PWA */}
            <div className="border border-[var(--brand-border)] rounded-lg p-4 bg-[var(--brand-cream)]/50">
              <div className="flex items-center gap-2 mb-2">
                <Globe className="w-5 h-5 text-[var(--brand-red)]" />
                <span className="text-[13px] font-bold text-[var(--brand-ink)]">{t('about.pwa')}</span>
              </div>
              <p className="text-[11.5px] text-[var(--brand-ink)]/70 leading-relaxed mb-3">
                {t('about.pwa.desc')}
              </p>
              <a
                href="/"
                onClick={(e) => {
                  // PWA install is triggered by browser — show instructions
                  e.preventDefault()
                  alert(t('common.install_app') + ': ' + (typeof window !== 'undefined' && window.matchMedia('(display-mode: standalone)').matches
                    ? '✓ Already installed'
                    : 'Click the install icon (⊕) in your browser address bar, or use the "Install App" button in the header.'))
                }}
                className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-[var(--brand-red)] text-white text-[11px] font-medium rounded-md hover:bg-[var(--brand-red-dark)] transition-colors"
              >
            ...
            <div className="border border-[var(--brand-border)] rounded-lg p-4 bg-[var(--brand-cream)]/50">
              <div className="flex items-center gap-2 mb-2">
                <Smartphone className="w-5 h-5 text-[var(--brand-red)]" />
                <span className="text-[13px] font-bold text-[var(--brand-ink)]">{t('about.android')}</span>
              </div>
              <p className="text-[11.5px] text-[var(--brand-ink)]/70 leading-relaxed mb-3">
                {t('about.android.desc')}
              </p>
              <div className="text-[10.5px] text-[var(--brand-ink)]/60 space-y-0.5 mb-3">
                <div>{t('about.android.version')}</div>
                <div>{t('about.android.size')}</div>
            ...
            <div className="border border-[var(--brand-border)] rounded-lg p-4 bg-[var(--brand-cream)]/50">
              <div className="flex items-center gap-2 mb-2">
                <Apple className="w-5 h-5 text-[var(--brand-red)]" />
                <span className="text-[13px] font-bold text-[var(--brand-ink)]">{t('about.ios')}</span>
              </div>
              <p className="text-[11.5px] text-[var(--brand-ink)]/70 leading-relaxed mb-3">
                {t('about.ios.desc')}
              </p>
              <details className="text-[11px]">
                <summary className="cursor-pointer text-[var(--brand-red)] font-medium hover:underline">
                  {t('about.ios.steps')}
                </summary>
                <ol className="mt-2 space-y-1 text-[var(--brand-ink)]/80 pl-1">
                  <li>{t('about.ios.step1')}</li>
                  <li>{t('about.ios.step2')}</li>
                  <li>{t('about.ios.step3')}</li>
                  <li>{t('about.ios.step4')}</li>
                </ol>
              </details>
            </div>
          </div>
            ...
            <div className="flex items-start gap-2 mb-2">
              ...
              <div>
                <div className="text-[12.5px] font-bold text-amber-900">
                  {t('about.offline_title')}
                </div>
                <p className="text-[11.5px] text-amber-800/80 leading-relaxed mt-1">
                  {t('about.offline_desc')}
                </p>
              </div>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mt-3">
              <div>
                <div className="text-[10px] uppercase tracking-wider text-green-700 font-medium mb-1.5 flex items-center gap-1">
                  <Check className="w-3 h-3" /> Can do
                </div>
                <ul className="space-y-1">
                  <li className="text-[11px] text-amber-900/90 flex items-start gap-1.5">
                    <Check className="w-3 h-3 text-green-600 flex-shrink-0 mt-0.5" />
                    <span>{t('about.offline_can1')}</span>
                  </li>
                  <li className="text-[11px] text-amber-900/90 flex items-start gap-1.5">
                    <Check className="w-3 h-3 text-green-600 flex-shrink-0 mt-0.5" />
                    <span>{t('about.offline_can2')}</span>
                  </li>
                  <li className="text-[11px] text-amber-900/90 flex items-start gap-1.5">
                    <Check className="w-3 h-3 text-green-600 flex-shrink-0 mt-0.5" />
                    <span>{t('about.offline_can3')}</span>
                  </li>
                </ul>
              </div>
              <div>
                <div className="text-[10px] uppercase tracking-wider text-red-700 font-medium mb-1.5 flex items-center gap-1">
                  <X className="w-3 h-3" /> Cannot do
                </div>
                <ul className="space-y-1">
                  <li className="text-[11px] text-amber-900/90 flex items-start gap-1.5">
                    <X className="w-3 h-3 text-red-600 flex-shrink-0 mt-0.5" />
                    <span>{t('about.offline_cant1')}</span>
                  </li>
                  <li className="text-[11px] text-amber-900/90 flex items-start gap-1.5">
                    <X className="w-3 h-3 text-red-600 flex-shrink-0 mt-0.5" />
                    <span>{t('about.offline_cant2')}</span>
                  </li>
                  <li className="text-[11px] text-amber-900/90 flex items-start gap-1.5">
                    <X className="w-3 h-3 text-red-600 flex-shrink-0 mt-0.5" />
                    <span>{t('about.offline_cant3')}</span>
                  </li>
                </ul>
              </div>
            </div>
